...
steps <- read_excel("data/steps.xlsx")
```
...
```{r}
#| warning: false
steps_summary <- steps_clean |> 
  group_by(Shift) |> 
  summarize(mean = mean(number_of_steps),
            ci = mean_cl_normal(number_of_steps)) |> 
  mutate(mean = round(mean, 1),
         ci = round(ci, 1)) |> 
  gt()
gt_tbl <- steps_summary |> 
    tab_header(title = "Steps Summary")
